...
export const GdpAdmin: React.FC<GdpAdminProps> = ({
  ...
  const fetchGdpData = async () => {
    setLoading(true);
    try {
      const [valueResult, growthResult, annualValueResult, annualGrowthResult] = await Promise.all([
  ...
  const downloadTemplate = (type: 'value' | 'growth' | 'annual-value' | 'annual-growth') => {
  ...
  const validateAnnualValueRow = (row: any): { isValid: boolean; errors: string[] } => {
    const errors: string[] = [];
  ...
  const validateAnnualGrowthRow = (row: any): { isValid: boolean; errors: string[] } => {
    const errors: string[] = [];
  ...
  const processExcelFile = async (file: File, type: 'value' | 'growth' | 'annual-value' | 'annual-growth') => {
    setUploading(true);
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>, type: 'value' | 'growth' | 'annual-value' | 'annual-growth') => {
