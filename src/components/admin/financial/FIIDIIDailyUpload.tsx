...
export function FIIDIIDailyUpload() {
  const [file, setFile] = useState<File | null>(null);
  const [uploading, setUploading] = useState(false);
  const [previewData, setPreviewData] = useState<DailyDataRow[]>([]);
  ...
  const parseFile = async (file: File) => {
    try {
      ...
      const jsonData = XLSX.utils.sheet_to_json<DailyDataRow>(worksheet);
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i += 100) {
        ...
        const { error } = await supabase
          ...
          .upsert(batch, { onConflict: 'date,investor_type,segment,asset_class' });
      ...
      toast({
        title: 'Success',
        description: `Successfully uploaded ${rows.length} daily records`,
      });
  ...
  const handleDownloadTemplate = () => {
    window.open('/templates/fii_dii_daily_template.csv', '_blank');
  };

  return (
    <Card>
      <CardHeader>
        <CardTitle>Daily FII/DII Data Upload</CardTitle>
        <CardDescription>
          Upload daily detailed FII and DII flows by segment and asset class
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="flex items-center justify-between p-4 bg-muted rounded-lg">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Get the CSV template for daily data</p>
          </div>
        ...
        <div className="space-y-2">
          <Label htmlFor="daily-file">Upload CSV/Excel File</Label>
          <Input
            id="daily-file"
            type="file"
            accept=".xlsx,.xls,.csv"
                ...
                <tbody>
                  {previewData.map((row, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="p-2 border">{row.Segment}</td>
                      <td className="p-2 border">{row.Asset_Class}</td>
