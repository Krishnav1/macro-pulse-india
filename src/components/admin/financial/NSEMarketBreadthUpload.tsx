// NSE Market Breadth Manual Upload Component
...
export function NSEMarketBreadthUpload() {
      ...
      Papa.parse(file, {
        ...
        complete: async (results) => {
          const data = results.data.map((row: any) => ({
            ...
            advances: parseInt(row['Advances']),
            declines: parseInt(row['Declines']),
            unchanged: parseInt(row['Unchanged']),
            high_52w: parseInt(row['52W High']),
            low_52w: parseInt(row['52W Low']),
            total_stocks: parseInt(row['Total Stocks']),
            timestamp: new Date(row['Date']).toISOString(),
          }));

          const { error } = await (supabase as any)
            .from('market_breadth')
            .upsert(data, { onConflict: 'date,exchange' });

          if (error) throw error;

          toast({
            title: 'Upload Successful',
            description: `${data.length} market breadth records uploaded`,
          });
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Upload Market Breadth Data
          </CardTitle>
          <CardDescription>
            Upload CSV file with market breadth metrics. Download template for correct format.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div>
            <a
              href="/templates/nse_market_breadth_template.csv"
              download
      ...
      <Card className="bg-primary/5 border-primary/20">
        <CardContent className="pt-6">
          <p className="text-sm text-foreground">
            <strong>Note:</strong> CSV should contain: Date, Exchange, Advances, Declines, Unchanged, 52W High, 52W Low, Total Stocks.
            Market breadth shows the number of advancing vs declining stocks.
          </p>
        </CardContent>
      </Card>
    </div>
